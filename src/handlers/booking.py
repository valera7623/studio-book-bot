...
async def cb_pick_date(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    if len(durations) == 1:
        await _show_slots(callback.message, session, resource, day, durations[0], edit=True)
        await callback.answer()
        return
    await _set_message(
        callback.message,
...
async def _show_slots(
    ...
    *,
    ...
    if not slots:
        await _set_message(message, "На эту дату свободных слотов нет.", edit=edit)
        return
...
async def cb_pick_duration(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    if not slots:
        ...
        return
    await _show_slots(callback.message, session, resource, day, duration_min, edit=True)
    await callback.answer()
...
async def cb_pick_slot(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    await _set_message(callback.message, f"Слот {when}{extra}. Как вас зовут?", edit=True)
    await callback.answer()
