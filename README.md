...
- Публичная запись по `t.me/bot?start=<slug>`: зал → дата → длительность → слот → контакты → согласие ПДн → hold.
...
|---|---|---|
| Клиент студии | `t.me/bot?start=<slug>` или `start=book_<id>` | Публичная запись, `/my` отмена, без пароля |
