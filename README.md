...
| Владелец студии | Онбординг и привязка Telegram | Кабинет: слоты, брони, отмена, часы, тексты. Сколько клиентов — шапка `/studio` и `/stats`. Super Admin не нужен |
| Админ платформы | `ADMINS` в `.env` | `/admin` — саппорт продукта и подписчики тарифа (старт/плюс). Отдельной роли Super Admin нет |
